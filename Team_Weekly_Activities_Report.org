...
LM size.: It looks llike this is not the problem. I made a smaller LM with only the prompts and the GPU memory problem still occurrs.
Hidden Layer Dimension: I lowered these from 512 to 64 and the GPU memory  problem did not occurr, so this is a candidate.
Chunk mini batch sizes: I lowered tis to 64,32,16 and the GPU memory problem went away, so this is a candidate.
Chunk width: I halved this parameter to 70,50,80 and the training succeeded. This is my favoritte candidate now.

- TODO Babel Multilang: Investigate how to implement these recipes. 

- Accent ID: This week I started setting up a recipe for Accent Id.
